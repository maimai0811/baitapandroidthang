...
public class HomeFragment extends Fragment implements View.OnClickListener {
    ...
    //Chart
    private BarChart lcThongKeThuChi;
    //Dữ liệu
    ...
    private void lineChart() {
        ...
        List<BarEntry> listPie = new ArrayList<>();
        if (tongThu > 0) {
            listPie.add(new BarEntry(0, tongThu));
        }

        // Check if chi is greater than 0 and add it to the entries
        if (tongChi > 0) {
            listPie.add(new BarEntry(1, tongChi));
        }

        barDataSet = new BarDataSet(listPie, null);
        barDataSet.setColors(ColorTemplate.MATERIAL_COLORS);
        barDataSet.setValueTextColor(Color.BLACK);
        lcThongKeThuChi.getDescription().setEnabled(false);
        lcThongKeThuChi.getAxisRight().setEnabled(false);
        lcThongKeThuChi.getAxisLeft().setEnabled(false);
        lcThongKeThuChi.getAxisLeft().setAxisMinValue(0f);
        lcThongKeThuChi.getLegend().setEnabled(false);
        lcThongKeThuChi.getXAxis().setDrawGridLines(false);
        lcThongKeThuChi.animateY(500);

        xAxis = lcThongKeThuChi.getXAxis();
        xAxis.setPosition(XAxis.XAxisPosition.BOTTOM);
        xAxis.setTextColor(Color.BLACK);
        String[] titles = {"Tổng thu", "Tổng chi"};
        ...
        lcThongKeThuChi.setDrawGridBackground(false);
        lcThongKeThuChi.setDrawBarShadow(false);
        barData = new BarData(barDataSet);
        barData.setBarWidth(1f);
        lcThongKeThuChi.setData(barData);
...
//        BarDataSet thuchi = new BarDataSet(listPie, "");
